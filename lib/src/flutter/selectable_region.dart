...
class SelectableRegionState extends State<SelectableRegion>
  ...
  void initState() {
    ...
    if (!kReleaseMode) {
      HardwareKeyboard.instance.addHandler(_debugCopyKeyDiagnostic);
    }
  ...
  /// Diagnostic-only hardware keyboard tap. Never consumes events.
  ///
  /// If ⌘C is pressed and this log does NOT appear, macOS consumed the key
  /// equivalent natively (e.g. the Edit ▸ Copy menu item / first responder
  /// `copy:`) and the event never entered the Flutter framework at all.
  bool _debugCopyKeyDiagnostic(KeyEvent event) {
    if (event is KeyDownEvent &&
        event.logicalKey == LogicalKeyboardKey.keyC &&
        HardwareKeyboard.instance.isMetaPressed) {
      copyDiagnosticLog(() =>
          'hardware ⌘C reached Flutter: regionHasFocus=${_focusNode.hasFocus} '
          'selectionStatus=${_selectionDelegate.value.status} '
  ...
  void _updateSelectionStatus() {
    ...
    // rationale: was in flutter code when copied on 25-11-12.
  ...
  void _handleMouseDragEnd(TapDragEndDetails details) {
    ...
    final bool isPointerPrecise =
        _isPrecisePointerDevice(_lastPointerDeviceKind!);
  ...
  void _onAnyDragEnd(DragEndDetails details) {
    final bool draggingHandles = _selectionOverlay != null &&
  ...
  void _handleSelectionStartHandleDragStart(DragStartDetails details) {
    ...
    _selectionOverlay!.showMagnifier(
      _buildInfoForMagnifier(details.globalPosition,
  ...
  void _handleSelectionEndHandleDragStart(DragStartDetails details) {
    ...
    _selectionOverlay!.showMagnifier(
      _buildInfoForMagnifier(
          details.globalPosition, _selectionDelegate.value.endSelectionPoint!),
    );
    _updateSelectedContentIfNeeded();
  }
  ...
  Future<void> _copy() async {
    // copyDiagnosticLog is intentionally always-on (debug builds): Cmd+C
    // failures are silent, so we need breadcrumbs to tell apart
    // "key never reached the region" vs "serialization failed" vs
    // "copied empty text".
    try {
      final SelectedContent? data = _selectable?.getSelectedContent();
      if (data == null) {
        copyDiagnosticLog(() => '_copy: getSelectedContent returned null '
            '(selectable=${_selectable != null})');
        return;
      }
      copyDiagnosticLog(() => '_copy: copying ${data.plainText.length} chars');
      if (data.plainText.isEmpty) {
        copyDiagnosticLog(() => '_copy: WARNING copied empty string');
      }
      await Clipboard.setData(ClipboardData(text: data.plainText));
    } catch (e, st) {
      copyDiagnosticLog(() => '_copy: THREW $e\n$st');
      rethrow;
    }
  }
  ...
  void dispose() {
    if (!kReleaseMode) {
      HardwareKeyboard.instance.removeHandler(_debugCopyKeyDiagnostic);
    }
    _selectable?.removeListener(_updateSelectionStatus);
    _selectable?.pushHandleLayers(null, null);
  ...
  Widget build(BuildContext context) {
    ...
    Widget result = SelectableRegionSelectionStatusScope._(
      ...
      child: SelectionContainer(
          registrar: this, delegate: _selectionDelegate, child: widget.child),
    );
    if (kIsWeb) {
...
class _CopySelectionAction extends _NonOverrideAction<CopySelectionTextIntent> {
  ...
  void invokeAction(CopySelectionTextIntent intent, [BuildContext? context]) {
    copyDiagnosticLog(() => '_CopySelectionAction invoked '
